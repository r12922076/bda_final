(function(){
  ...
  function diagnosis(){
    ...
    const scoreRules = breakdown.gaps.map(g=>({component:g.label, category:g.category, points:g.points, status:g.ok?"met":"missing"}));
    const body = `<section class="grid score-layout"><div class="card score-card"><div class="score-ring big"><span>${breakdown.total}</span><small>/100</small></div><h2>Strategy Health Score</h2><p class="muted">Workflow completeness score, not performance prediction.</p><div class="actions centered">${PG.routeButton("Generate Full Report", "report", "primary")}${PG.routeButton("Open Monitor", "monitor", "secondary")}</div></div><div class="card"><h2>Score breakdown</h2>${Object.entries(breakdown.categories).map(([k,v])=>PG.progress(k, v.score, v.max)).join("")}</div></section>
      <section class="card wide"><h2>Why these weights?</h2><p>The score uses fixed workflow-completeness weights: backtest realism 25, live alert observability 20, repaint/lookahead review 20, risk-rule completeness 20, and evidence quality 15. A high score means the workflow is documented enough to evaluate; it does not mean the strategy is profitable.</p>${PG.table(scoreRules,[{key:"component",label:"Component"},{key:"category",label:"Category",small:true},{key:"points",label:"Points"},{key:"status",label:"Status",render:r=>PG.badge(r.status,r.status==='met'?'green':'amber')}])}<div class="actions">${PG.routeButton("Open scoring documentation", "docs", "secondary")}</div></section>
  ...
  function docs(){
    const links=[
      ...
      ["Scoring rules","scoring_rules.md","Health-score weights and product-boundary interpretation."],
